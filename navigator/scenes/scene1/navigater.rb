...
class Navigater
  ...
  def run_forward(speed=WHEEL_SPEED)
    operate do
      @brick.step_velocity(speed, 242, 40, *wheel_motors)
  ...
  def turn_right(speed=WHEEL_SPEED)
    operate do
      ...
      @brick.step_velocity(speed, 123, 60, *wheel_motors)
      @brick.motor_ready(*wheel_motors)
    end
  end
 ...
 #ちょっと前に進みたいとき
  def run_forwardX(speed=WHEEL_SPEED)
    operate do
      @brick.step_velocity(speed, 100, 40, *wheel_motors)
      @brick.motor_ready(*wheel_motors)
    end
  end
